package stockingproblem;

import algorithms.Problem;

import java.io.File;
import java.io.IOException;
import java.util.ArrayList;
//numero de linhas e numero de colunas sao os 2 primeiros valores do dataSet

//onde eu tenho todas as peças
public class StockingProblem implements Problem<StockingProblemIndividual> {
    private int materialHeight;//altura
    private int materialWidth;//largura
    private ArrayList<Item> items;//peças

    public StockingProblem(int materialHeight, ArrayList<Item> items) {
        this.materialHeight = materialHeight;
        this.items = items;

        //TODO - DONE
        //pecorrer todos os itens, criar um atributo que tenha tudo
        for (int i = 0; i < items.size(); i++){
            //para cada item guarda o numero de itens e coluna
            materialWidth += items.get(i).getColumns();
        }
        //criar uma matriz "estatica" de acordo com o tamanho dos itens pecorridos
    }

    @Override
    public StockingProblemIndividual getNewIndividual() {
        //TODO - DONE
        //pouca coisa para fazer nesse método
        //esse metodo vai criar um novo individuo
        return new StockingProblemIndividual(this,items.size());//pq eu quero o tamanho
        //MÉTODO chamado na construção de cada individuo da população(population.java)
        //throw new UnsupportedOperationException("Not implemented yet.");
    }

    public int getMaterialWidth() {
        return materialWidth;
    }

    public int getMaterialHeight() {
        return materialHeight;
    }

    public ArrayList<Item> getItems() {
        return items;
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("Raw material height: ")
                .append(materialHeight);
        sb.append("\nNumber of items: ").append(items.size());

        sb.append("\n\nItems: \n");
        for (Item item : items) {
            sb.append("\nPiece ").append(item.getId()).append("-").append(item.getRepresentation())
                    .append(item);
        }
        return sb.toString();
    }

    public static StockingProblem buildWarehouse(File file) throws IOException {

        java.util.Scanner f = new java.util.Scanner(file);
        int materialHeight = f.nextInt();//primeiro valor do ficheiro
        int numberOfItems = f.nextInt();//segundo valor -> numero de peças
        ArrayList<Item> items = new ArrayList<>();
        for (int i = 0; i < numberOfItems; i++) {
            int itemLines = f.nextInt();
            int itemColumns = f.nextInt();
            if (itemLines > materialHeight) {
                throw new RuntimeException("Invalid item, cannot be fitted to specified material");
            }
            int[][] matrix = new int[itemLines][itemColumns];
            for (int j = 0; j < itemLines; j++) {
                for (int k = 0; k < itemColumns; k++) {
                    matrix[j][k] = f.nextInt();
                }
            }
            items.add(new Item(i, matrix));
        }
        return new StockingProblem(materialHeight, items);
    }
}
